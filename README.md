...
## Project Structure
AI‑Story‑and‑Poem‑Generator/ (git repo root) ├─ backend/ # FastAPI server │ ├─ main.py │ ├─ requirements.txt │ └─ .env # LLM service URL (edit this) ├─ frontend/ # Vite + React app │ ├─ src/ │ │ ├─ App.jsx │ │ ├─ components/StoryGenerator.jsx │ │ └─ components/StoryGenerator.css │ ├─ index.css │ ├─ package.json │ └─ ... ├─ .gitignore └─ README.md # <-- this file
...
```bash
git clone [https://github.com/1-WaleedAhmad/AI-Story-and-Poem-Generator.git](https://github.com/1-WaleedAhmad/AI-Story-and-Poem-Generator.git)
...
Edit 
backend/.env
 and replace the placeholder URL with the actual URL of your Hugging Face Space endpoint:
...
Enter a theme/prompt (e.g., "rainy day").
Choose Story or Poem.
Adjust temperature / top‑k / top‑p if desired.
Click Generate – the result will appear below.
Important Note
You must paste the URL of your Hugging Face Space into 
backend/.env
 (the LLM_SERVICE_URL variable). Without this, the backend cannot forward the request and the frontend will show an error.
